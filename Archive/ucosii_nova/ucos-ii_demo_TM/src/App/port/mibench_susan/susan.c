...
#define  exit_error(IFB,IFC) { xil_printf(IFB,IFC); exit(0); }
...
usage()
{
  print("Usage: susan <in.pgm> <out.pgm> [options]\n\n\r");

  print("-s : Smoothing mode (default)\n\r");
  print("-e : Edges mode\n\r");
  print("-c : Corners mode\n\n\r");

  print("See source code for more information about setting the thresholds\n\r");
  print("-t <thresh> : Brightness threshold, all modes (default=20)\n\r");
  print("-d <thresh> : Distance threshold, smoothing mode, (default=4) (use next option instead for flat 3x3 mask)\n\r");
  print("-3 : Use flat 3x3 mask, edges or smoothing mode\n\r");
  print("-n : No post-processing on the binary edge map (runs much faster); edges mode\n\r");
  print("-q : Use faster (and usually stabler) corner mode; edge-like corner suppression not carried out; corners mode\n\r");
  print("-b : Mark corners/edges with single black points instead of black with white border; corners or edges mode\n\r");
  print("-p : Output initial enhancement image only; corners or edges mode (default is edges mode)\n\r");
...
/* }}} */
  ...
  *in = (uchar *) malloc(*x_size * *y_size);
  ...
  if(f_read(fd,*in,*x_size * *y_size, &cnt)!= FR_OK)
...
int br;
  ...
  if(f_write(fd, in, x_size*y_size, &br ) != FR_OK)
...
/* }}} */
  ...
  if ( (dt>15) && (total==0) )
  {
    xil_printf("Distance_thresh (%f) too big for integer arithmetic.\n\r",dt);
    xil_printf("Either reduce it to <=15 or recompile with variable \"total\"\n\r");
    xil_printf("as a float: see top \"defines\" section.\n\r");
    exit(0);
  }

  if ( (2*mask_size+1>x_size) || (2*mask_size+1>y_size) )
  {
    xil_printf("Mask size (1.5*distance_thresh+1=%d) too big for image (%dx%d).\n\r",mask_size,x_size,y_size);
    exit(0);
  }
...
/* }}} */
  ...
  int   argc = 4;
  //char  *argv [4]= {"susan", "ssin.pgm", "sout.pgm", "-s"};
  char  *argv [4]= {"susan", "ssin.pgm", "sout.pgm", "-e"};
  //char  *argv [4]= {"susan", "ssin.pgm", "sout.pgm", "-c"};
